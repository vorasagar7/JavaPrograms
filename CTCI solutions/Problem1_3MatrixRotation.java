...
import java.util.*;
class Problem1_3MatrixRotation
{
	public static boolean rotation(int matrixArray[][])
	{
		...
		for(int layers=0;layers<n/2;layers++)
		{
			...
			for(int i=first;i<last;i++)
			{
				...
				matrixArray[first][i] = matrixArray[last-offset][first];
				matrixArray[last-offset][first] = matrixArray[last][last-offset];
				matrixArray[last][last-offset] = matrixArray[i][last];
				matrixArray[i][last] = top;
			}
		}
		return true;
	}
